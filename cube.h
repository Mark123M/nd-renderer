...
struct cube : public shape {
	...
	__host__ __device__ cube(): half_len{ 0.25f } {}
	...
	__host__ __device__ bool intersect(const ray& r, hit_result& res) const override {
        ...
        if (approx_equals(local_r_pos.w, 0.f)) {
            if (!approx_equals(local_r_dir.w, 0.f)) {
                return false;
            }
            // ray is in plane so proceed with slab test
        } else {
            // ray is out of plane so initialize t s.t. p.w = 0
            t_near = -local_r_pos.w / local_r_dir.w;
            t_far = t_near;
        }
